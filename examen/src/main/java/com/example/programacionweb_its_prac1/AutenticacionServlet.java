...
public class AutenticacionServlet extends HttpServlet {
    ...
    private void register(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        boolean usernameOrEmailInUse = users.values().stream()
                .anyMatch(user -> user.getUsername().equals(username) || user.getEmail().equals(email));

        if (usernameOrEmailInUse) {
            jResp.failed(req, resp, "El nombre de usuario o correo electrónico ya están en uso", 422);
            return;
        }
    ...
    private void login(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (user != null) {
            if (verifyPassword(password, user.getPassword())) {
                // usamos el objeto date para crear el tiempo de expiracion
                Date now = new Date();
                long t = now.getTime();
                Date tiempo_expiracion = new Date(t + (5 * 60 * 1000)); // 5 minutos

                String token = Jwts.builder()
                        .setHeaderParam("kid", SECRET_KEY)
                        .setSubject(username)
                        .setExpiration(tiempo_expiracion) // Agregar la fecha de expiración
                        .signWith(generalKey())
                        .compact();
